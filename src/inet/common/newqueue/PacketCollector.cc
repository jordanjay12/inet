...
void PacketCollector::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        inputGate = gate("in");
        provider = check_and_cast<IPacketProvider *>(gate("in")->getPathStartGate()->getOwnerModule());
...
{
    scheduleAt(simTime() + par("collectionInterval"), collectionTimer);
}
...
void PacketCollector::notifyCanPopPacket(cGate *gate)
{
